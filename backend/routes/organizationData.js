...
let { orgdata } = require("../models/models"); //imports the primarydata model from models.js}
...
router.get("/", (req, res, next) => { // route method, path, followed by callback
    orgdata.find( //model.find({filters}) finds documents, since we have no filters, it returns everything
...
router.get("/:id", (req, res, next) => { // route method, path followed by a parameter, callback
    orgdata.findOne({_id: req.params.id},(error, data) => {
        if (error) {
          return next(error)
        } else {
          res.json(data)
...
router.post("/", (req, res, next) => { // route method, path, callback 
    orgdata.create( //model.create() creates a new document using primarydata model
...
router.put("/:id", (req, res, next) => { // route method, path followed by parameter, callback 
    orgdata.findOneAndUpdate( //model.findOneAndUpdate({filter}) finds and updates a document based on what arguements you pass. filters are contained within brackets 
