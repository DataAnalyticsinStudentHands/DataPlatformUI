/**
 * src/components/studentSide/projectView/forms/shared/FileUploader.vue
 *
...
async function validateAndSetFile(file) {
  ...
  // Magic-number validation for images and PDFs
  if (isPdf || isImage) {
    const detected = await validateImageSignature(file);
    if (detected === null) {
      errorMessage.value = 'File content does not match the expected type. The file may be corrupted or renamed.';
      return;
    }
  }
